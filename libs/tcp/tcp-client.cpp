...
{
    *instance_ptr = (TCPClient *) uv_handle_get_data(handle);
}
...
static void _OnWriteBuffer(uv_write_t* request, int status) 
{
    ...
    free(instance->temp_write_buffer.base);
    memset(&instance->temp_write_buffer, 0, sizeof(uv_buf_t));
...
void TCPClient_Write(TCPClient *instance, const uint8 data[], uint32 data_size)
{
    ...
    char *data_alloced = (char *) malloc(sizeof(char) * data_size);
...
{
    TCPClient *instance = (TCPClient *) malloc(sizeof(TCPClient));
    ...
    memset(&instance->server_addr, 0, sizeof(instance->server_addr));
    memset(&instance->uv_connection, 0, sizeof(instance->uv_connection));
    memset(&instance->temp_write_buffer, 0, sizeof(instance->temp_write_buffer));
    memset(&instance->temp_write_request_handle, 0, sizeof(instance->temp_write_request_handle));
